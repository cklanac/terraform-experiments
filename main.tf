...
// Fetches the Google project information based on provided project ID
data "google_project" "project" {
  project_id = var.project_id
}

// Defines local variables for container images
locals {
  api_image = "gcr.io/sic-container-repo/todo-api-postgres:latest"
  fe_image  = "gcr.io/sic-container-repo/todo-fe"
}

// Module to enable Google Cloud APIs in the project
...
// Creates a Google Service Account for Cloud Run
...
// Assigns roles to the service account
...
// Sets up a global IP address for VPC peering
...
// Establishes a service networking connection for peering
...
// Redis instance configuration for caching
...
// Sets up a PostgreSQL database instance
...
// Sets up a Cloud Run service for the API
resource "google_cloud_run_service" "api" {
  ...
  // Defines the container template for the API service
  template {
    spec {
    ...
    // Metadata and annotations for the API service
    metadata {
      annotations = {
...
// Configures a Cloud Run service for the frontend
resource "google_cloud_run_service" "fe" {
  name     = "${var.deployment_name}-fe"
  location = var.region
  project  = var.project_id

  // Container template for the frontend service
  template {
    spec {
      service_account_name = google_service_account.runsa.email
      containers {
...
// Grants public access to the API Cloud Run service
...
// Public access setup for the Cloud Run frontend service
